...
/* Dingwen */
#undef __FUNCT__
#define __FUNCT__ "KSPSetOnlineABFT"
PetscErrorCode KSPSetOnlineABFT(KSP ksp,PetscInt solver_type,PetscInt error_type, PetscInt itv_c, PetscInt itv_d, PetscInt inj_itr,PetscInt inj_times,PetscInt inj_num)
{
  PetscErrorCode ierr;
 
  PetscFunctionBegin;
  ksp->solver_type = solver_type;
  ksp->error_type = error_type;
  ksp->itv_c = itv_c;
  ksp->itv_d = itv_d;
  ksp->inj_itr = inj_itr;
  ksp->inj_times = inj_times;
  ksp->inj_num = inj_num;
  PetscFunctionReturn(0);
}
/* Dingwen */