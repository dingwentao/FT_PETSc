...
int main(int argc,char **args)
{
  ...
  for (i=0; i<nnz; i++) {
    ierr = MatSetValues(A,1,&J[i],1,&II[i],&VAL[i],INSERT_VALUES);CHKERRQ(ierr);
  }
  ...
  ierr = PetscFree4(II,J,VAL,rownz);CHKERRQ(ierr);
